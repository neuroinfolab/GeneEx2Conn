...
def plot_connectome_predictions_full(Y, Y_true, Y_pred, indices, network_labels=None, binarize=False, config=None, region_pair_dataset=None, shared_indices=None):
    ...
    save_pred_matrix = False # default is to skip prediction matrix saving
        
    if save_pred_matrix and 'Test' in config.mode_title: # for spatial/random
        print(f"Config mode title: {config.mode_title}")
        print(f"Save pred matrix: {save_pred_matrix}")
        print(f"Test in config mode title: {'Test' in config.mode_title}")

        print(f"Saving pred matrix for {config.mode_title}")
        ...
        fold = 0
        save_dir = f"notebooks/OHBM/preds/"
        os.makedirs(save_dir, exist_ok=True)
        while os.path.exists(f"{save_dir}/pred_matrix_test_fold{fold}.npy"):
            fold += 1
        np.save(f"{save_dir}/pred_matrix_test_fold{fold}.npy", pred_matrix)

    # if save_pred_matrix: # for schaefer
    #     print(f"Split mask type: {type(split_mask)}")
    #     print(f"Split mask shape: {split_mask.shape}")
    #     print("Split mask:")
    #     print(split_mask)
