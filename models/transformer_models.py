...
# === FAST SELF-ATTENTION BLOCK WITH CONV1D QKV PROJECTIONS AND RMSNORM === #
class FastSelfAttentionBlockConv(nn.Module):
    def __init__(self, d_model, nhead, dropout=0.1, use_alibi=False):
        super().__init__()
        self.nhead = nhead
        self.head_dim = d_model // nhead
...
# === SelfAttentionCLSEncoderConv using FastSelfAttentionBlockConv, FlashAttention, and CLS token === #
class SelfAttentionCLSEncoderConv(nn.Module):
    def __init__(self, token_encoder_dim, d_model, output_dim, nhead=4, num_layers=4, dropout=0.1, cls_init='spatial_learned', use_alibi=False):
        ...
        # Transformer layers
        self.layers = nn.ModuleList([
            FastSelfAttentionBlockConv(d_model, nhead, dropout, use_alibi=self.use_alibi)
            for _ in range(num_layers)
        ])

        # Register per-head ALiBi slopes once if use_alibi is True
        if self.use_alibi:
            slopes = FastSelfAttentionBlockConv.build_alibi_slopes(nhead)
            self.register_buffer("alibi_slopes", slopes)
            for layer in self.layers:
                layer.alibi_slopes = self.alibi_slopes
