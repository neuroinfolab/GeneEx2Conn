...
class BaseTransformerModel(nn.Module):
    def __init__(self, input_dim, learning_rate=0.001, weight_decay=0.0, batch_size=256, epochs=100, num_workers=2, prefetch_factor=2):
        super().__init__()
    ...
    def predict(self, loader, collect_attn=False, save_attn_path=None):
        """Shared predict function for all transformer models"""
        self.eval()
        predictions = []
        targets = []
...
class FlashAttentionBlockConv(nn.Module):
    ...
    def forward(self, x):
        with autocast(dtype=torch.bfloat16):
            ...
            qkv = self.qkv_proj(x_conv).transpose(1, 2)
...
class SelfAttentionCLSEncoderConv(nn.Module):
    ...
    def forward(self, gene_exp, coords):
        ...
        gene_exp = gene_exp.view(batch_size, L, self.token_encoder_dim).transpose(1, 2)
        x_proj = self.input_projection(gene_exp).transpose(1, 2)
...
class SharedSelfAttentionConvCLSModel(BaseTransformerModel):
    def __init__(self, input_dim, token_encoder_dim=20, d_model=128, encoder_output_dim=10, nhead=2, num_layers=2, deep_hidden_dims=[256, 128],
                 cls_init='spatial_learned', use_alibi=False, transformer_dropout=0.1, dropout_rate=0.1, learning_rate=0.001, weight_decay=0.0,
    ...
    def forward(self, x, coords):
        ...
        pred = torch.clamp(output, min=-0.8, max=1.0)
        return pred.squeeze()
    ...
    def predict(self, loader, collect_attn=False, save_attn_path=None):
        """Override predict for Conv model (only supports full attention)"""
        self.eval()
        predictions = []
        targets = []
        total_batches = 0
