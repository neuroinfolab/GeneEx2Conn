...
def plot_final_subsetted_barchart(
    ...
    # Add neat gridlines behind bars - gridlines every 0.1, ticks every 0.2
    grid_interval = 0.1  # Finer gridlines
    tick_interval = 0.2  # Standard tick spacing
    if horizontal:
        # Y-axis: Set major ticks for labels (0.2) and minor ticks for gridlines (0.1)
        ax.yaxis.set_major_locator(plt.MultipleLocator(tick_interval))
        ax.yaxis.set_minor_locator(plt.MultipleLocator(grid_interval))
        ax.grid(True, axis='y', alpha=0.3, which='both')  # Show both major and minor gridlines
        ax.tick_params(axis='y', which='minor', length=0)  # Hide minor tick marks but keep gridlines
        ax.set_axisbelow(True)  # Force grid to appear below all plot elements
    else:
        # X-axis: Set major ticks for labels (0.2) and minor ticks for gridlines (0.1)
        ax.xaxis.set_major_locator(plt.MultipleLocator(tick_interval))
        ax.xaxis.set_minor_locator(plt.MultipleLocator(grid_interval))
        ax.grid(True, axis='x', alpha=0.3, which='both')  # Show both major and minor gridlines
        ax.tick_params(axis='x', which='minor', length=0)  # Hide minor tick marks but keep gridlines
        ax.set_axisbelow(True)  # Force grid to appear below all plot elements
