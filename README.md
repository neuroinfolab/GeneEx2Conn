...
## Setup instructions
- To use this repo, setup a conda environment using `GeneEx2Conn_env_combined.yml` from 'env' directory. If there are missing packages delete and recreate the environment with `GeneEx2Conn_env_all.yml`. Then setup ENIGMA as specified in 'Developer notes' below. If necessary, manually install the latest version of torch for MLP functionality, `pip3 install torch torchvision torchaudio`.
...
## Setting up Jupyter notebook on HPC (Greene)
...
Specifically, all you should need to do is 
