...
def load_transcriptome(parcellation='S456', gene_list='0.2', dataset='AHBA', run_PCA=None, omit_subcortical=False, hemisphere='both', impute_strategy='mirror_interpolate', sort_genes='refgenome', return_valid_genes=False, null_model='none', random_seed=42):
    ...
    if dataset == 'AHBA':
        ...
        if 'var' in gene_list: # ~250 most variable genes of 0.2 across brain
        ...
        if null_model == 'spin' and parcellation == 'S456':
            ...
            seed_to_index = {1: 0, 2: 1, 3: 2, 4: 3, 5: 4, 6: 5, 7: 6, 8: 7, 9: 8, 42: 9}
            # Add mappings for seeds 10 to 1000 (excluding any already present)
            next_idx = max(seed_to_index.values()) + 1
            for seed in range(10, 1001):
                if seed not in seed_to_index:
                    seed_to_index[seed] = next_idx
                    next_idx += 1
    ...
    if dataset == 'UKBB':
        ...
        if parcellation == 'S100':
            ...
            if measure == 'FC':
                ...
                matrix = np.array(pd.read_csv(f'./data/HCP1200/HCP1200_{parcellation}_FC_exp_decay.csv', header=None))
            elif measure == 'FC_EXP_RES':
                matrix = np.array(pd.read_csv(f'./data/HCP1200/HCP1200_{parcellation}_FC_exp_decay_residual.csv', header=None))
            elif measure == 'FC_FOCAL_DECAY':
                matrix = np.array(pd.read_csv(f'./data/HCP1200/HCP1200_{parcellation}_FC_focal_exp_decay.csv', header=None))
            elif measure == 'FC_FOCAL_RES':
                matrix = np.array(pd.read_csv(f'./data/HCP1200/HCP1200_{parcellation}_FC_focal_exp_decay_residual.csv', header=None))
            elif measure == 'SC':
                matrix = np.log1p(loadmat(f'./data/HCP1200/4{parcellation}_DTI_count.mat')['connectivity'])
                matrix = matrix / matrix.max()
        else:
