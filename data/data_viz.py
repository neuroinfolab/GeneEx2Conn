...
def define_density_train_test_splits(
    coords,
    num_test_coords,
    min_train_coords,
    max_train_coords,
    ...
    """
    Create deterministic coordinate splits for spatial-density experiments.

    Parameters:
    -----------
    coords : array-like, shape (n_coords, 3)
        Coordinate array used only to determine valid index range.
    num_test_coords : int
        Number of test coordinates to hold out.
    min_train_coords : int
        Minimum number of training coordinates in the first split.
    max_train_coords : int
        Maximum number of training coordinates in the last split.
    step_train_coords : int
        Increment for training coordinate count between splits.
    ...
    test_indices : np.ndarray
        Held-out test coordinate indices.
    train_splits : dict
        Dict mapping {n_train: np.ndarray of train indices}, where each train
        set is a strict superset of the previous one.
    """
    coords_arr = np.asarray(coords)
    if coords_arr.ndim != 2 or coords_arr.shape[1] != 3:
        raise ValueError("coords must have shape (n_coords, 3)")

    if step_train_coords <= 0:
        raise ValueError("step_train_coords must be > 0")
    if min_train_coords <= 0 or max_train_coords <= 0 or num_test_coords <= 0:
        raise ValueError("num_test_coords/min_train_coords/max_train_coords must be > 0")
    if min_train_coords > max_train_coords:
        raise ValueError("min_train_coords must be <= max_train_coords")

    n_coords = coords_arr.shape[0]
    if num_test_coords >= n_coords:
        raise ValueError("num_test_coords must be smaller than total number of coordinates")
    if num_test_coords + max_train_coords > n_coords:
        raise ValueError("num_test_coords + max_train_coords exceeds available coordinates")

    rng = np.random.default_rng(random_seed)
    all_indices = np.arange(n_coords)
    perm = rng.permutation(all_indices)

    test_indices = np.sort(perm[:num_test_coords])
    train_pool = perm[num_test_coords:]

    train_sizes = list(range(min_train_coords, max_train_coords + 1, step_train_coords))
    if train_sizes[-1] != max_train_coords:
        train_sizes.append(max_train_coords)

    train_splits = {}
    for n_train in train_sizes:
        train_splits[n_train] = np.sort(train_pool[:n_train])

    return test_indices, train_splits


def plot_density_train_test_splits(
    coords,
    test_indices,
    train_splits,
    view="coronal",
    max_plots_per_row=4,
    figsize_per_plot=(4.6, 4.2),
    train_color="#A89CAA",
    test_color="#FF7F0E",
    train_size=14,
    test_size=36,
    ...
    """
    Plot train/test coordinate splits across increasing train-density intervals.
    ...
    train_splits : dict
        Dict mapping {n_train: train_indices}.
    view : str, optional
        One of {"coronal", "sagittal", "axial", "3d"}.
    max_plots_per_row : int, optional
        Maximum number of subplots per row.
    figsize_per_plot : tuple, optional
        Figure size scaling per subplot (width, height).
    """
    coords_arr = np.asarray(coords)
    if coords_arr.ndim != 2 or coords_arr.shape[1] != 3:
        raise ValueError("coords must have shape (n_coords, 3)")
    ...
    view = view.lower()
    valid_views = {"coronal", "sagittal", "axial", "3d"}
    if view not in valid_views:
        raise ValueError(f"view must be one of {sorted(valid_views)}")

    split_items = sorted(train_splits.items(), key=lambda kv: kv[0])
    n_panels = len(split_items)
    n_cols = min(max_plots_per_row, n_panels)
    n_rows = int(np.ceil(n_panels / n_cols))

    fig = plt.figure(figsize=(figsize_per_plot[0] * n_cols, figsize_per_plot[1] * n_rows), dpi=220)

    view_to_dims = {
        "axial": (0, 1, "X (L-R)", "Y (P-A)"),
        "coronal": (0, 2, "X (L-R)", "Z (I-S)"),
        "sagittal": (1, 2, "Y (P-A)", "Z (I-S)"),
    }

    for panel_idx, (n_train, train_indices) in enumerate(split_items, start=1):
        train_indices = np.asarray(train_indices, dtype=int)

        if view == "3d":
            ax = fig.add_subplot(n_rows, n_cols, panel_idx, projection="3d")
            ax.scatter(
                coords_arr[train_indices, 0], coords_arr[train_indices, 1], coords_arr[train_indices, 2],
                c=train_color, s=train_size, alpha=alpha_train, edgecolor="none", label="Train"
            )
            ax.scatter(
                coords_arr[test_indices, 0], coords_arr[test_indices, 1], coords_arr[test_indices, 2],
                c=test_color, s=test_size, alpha=alpha_test, edgecolor="black", linewidth=0.3, label="Test"
            )
            ax.set_xlabel("X")
            ax.set_ylabel("Y")
            ax.set_zlabel("Z")
            ...
            d0, d1, xlab, ylab = view_to_dims[view]
            ax.scatter(
                coords_arr[train_indices, d0], coords_arr[train_indices, d1],
                c=train_color, s=train_size, alpha=alpha_train, edgecolor="none", label="Train"
            )
            ax.scatter(
                coords_arr[test_indices, d0], coords_arr[test_indices, d1],
                c=test_color, s=test_size, alpha=alpha_test, edgecolor="black", linewidth=0.3, label="Test"
            )
            ax.set_xlabel(xlab)
            ax.set_ylabel(ylab)
        ...
        ax.set_title(f"Test={len(test_indices)} | Train={len(train_indices)}", fontsize=10)

        if panel_idx == 1:
            ax.legend(loc="upper right", fontsize=8, frameon=False)

    # Hide any unused axes in last row
    total_slots = n_rows * n_cols
    for panel_idx in range(n_panels + 1, total_slots + 1):
        if view == "3d":
            ax = fig.add_subplot(n_rows, n_cols, panel_idx, projection="3d")
        else:
            ax = fig.add_subplot(n_rows, n_cols, panel_idx)
    ...
    fig.suptitle(f"Train/Test Coordinate Density Splits ({view})", fontsize=13, y=0.995)
    plt.tight_layout()
    plt.show()
...
def visualize_splits_3d_rotate(splits, coords, Y=None, X=None, edge_threshold=0.5, valid_genes=None, gene_name=None,
    ...
    for fold_idx, (train_indices, test_indices) in enumerate(splits, 1):
        ...
        for frame_idx, azimuth in enumerate(azimuth_angles):
            ...
            # Add fold indicator text above the plot
            fig.text(0.5, 0.85, f'Test Fold {fold_idx}', fontsize=fontsize, 
            ...
            # Remove margins for clean rotation, but leave space at top for text
...
def visualize_splits_3d_static(splits, coords, Y=None, X=None, edge_threshold=0.5, valid_genes=None, gene_name=None,
                               title_prefix="CV Split", show_train_edges=True, show_test_edges=True,
                               fontsize=25, include_legend=True):
    """
    Create static 3D visualizations of train/test splits with styling from the rotate function.
    ...
    include_legend : bool, optional
        Whether to include the legend in the plot
    ...
    # Colors
    red_orange = "#FF4500"    # For test edges
    orange = "#FFA500"        # For test nodes (classic orange)
    ...
    for fold_idx, (train_indices, test_indices) in enumerate(splits, 1):
        ...
        if Y is not None:
            ...
            for i, j in zip(*connected_pairs):
                if i < j:  # Only draw each edge once
                    ...
                    # Base alpha and width on connection strength (using rotate function styling)
                    base_alpha = norm_weight * 0.5  # Reduced alpha from rotate function
                    base_width = norm_weight * 1.7    # Reduced width from rotate function
                    ...
                    if show_train_edges and i in train_indices and j in train_indices:
                        ...
                        color = red_orange  # Edges: red-orange
                        alpha = base_alpha * 1.1
                        width = base_width * 1.75
                        ax.plot([start[0], end[0]], 
                               [start[1], end[1]], 
        ...
        # Plot points with gene expression coloring if available (using rotate function styling)
        if expression_values is not None:
            train_colors = create_color_gradient('#A89CAA', expression_values[train_indices])  # Light gray with pink hue
            test_colors = create_color_gradient(orange, expression_values[test_indices])       # Test nodes: orange
            
            train_scatter = ax.scatter(coords[train_indices, 0], 
                                     coords[train_indices, 1], 
                                     coords[train_indices, 2], 
                                     c=train_colors, label='Train', 
                                     s=70, edgecolor='gray', linewidth=0.5)  # Size from rotate function
            ...
            # Plot points with solid colors - light gray with pink hue for train (rotate function styling)
            train_scatter = ax.scatter(coords[train_indices, 0], 
                                     coords[train_indices, 1], 
                                     coords[train_indices, 2], 
                                     c='#A89CAA', label='Train', alpha=0.9,  # Color from rotate function
                                     s=70, edgecolor='gray', linewidth=0.7)  # Size from rotate function
            
            test_scatter = ax.scatter(coords[test_indices, 0], 
                                    coords[test_indices, 1], 
                                    coords[test_indices, 2], 
                                    c=orange, label='Test', alpha=0.9,        # Test nodes: orange
                                    s=70, edgecolor='gray', linewidth=0.5)  # Size from rotate function
        
        # Calculate axis limits for zooming in (make scatter plot appear larger)
        x_min, x_max = coords[:, 0].min(), coords[:, 0].max()
        y_min, y_max = coords[:, 1].min(), coords[:, 1].max()
        z_min, z_max = coords[:, 2].min(), coords[:, 2].max()
        
        # Apply zoom factor to make scatter plot bigger (reduce axis range by 15%)
        zoom_factor = 0.95
        x_center, y_center, z_center = (x_min + x_max) / 2, (y_min + y_max) / 2, (z_min + z_max) / 2
        x_range, y_range, z_range = (x_max - x_min) * zoom_factor, (y_max - y_min) * zoom_factor, (z_max - z_min) * zoom_factor
        
        ax.set_xlim([x_center - x_range/2, x_center + x_range/2])
        ax.set_ylim([y_center - y_range/2, y_center + y_range/2])
        ax.set_zlim([z_center - z_range/2, z_center + z_range/2])
        
        # Set axis labels on the inside of the plot (from original visualize_splits_3d)
        ax.set_xlabel('X (Lateral)', fontsize=fontsize, labelpad=-5)
        ax.set_ylabel('Y (Posterior-Anterior)', fontsize=fontsize, labelpad=-5)
        ax.set_zlabel('Z (Dorsal-Ventral)', fontsize=fontsize, labelpad=-5)
        
        # Enhanced axis styling for better depth perception
        # Make axis lines thicker and darker for more shadow/depth
        ...
        # Remove tick labels but keep marks (from original visualize_splits_3d)
        ax.set_xticklabels([])
        ax.set_yticklabels([])
        ax.set_zticklabels([])
        ax.tick_params(axis='both', which='major', labelsize=fontsize*0.6, width=2, length=6, colors='#333333')
        
        # Reduce number of ticks (from original visualize_splits_3d)
        for axis in [ax.xaxis, ax.yaxis, ax.zaxis]:
            axis.set_major_locator(plt.MaxNLocator(5))  # Show only 5 ticks per axis
        
        # Create legend with larger font and adjusted position (if requested)
        if include_legend:
            legend_elements = [
                plt.scatter([], [], c='#A89CAA', alpha=0.9, s=50),  # Updated color
                plt.scatter([], [], c=orange, alpha=0.9, s=50)      # Test nodes legend: orange
            ]
            legend_labels = ['Train Regions', 'Test Regions']
            
            # Adjust legend font size and position (from original visualize_splits_3d)
            ax.legend(legend_elements, legend_labels, 
                     fontsize=fontsize*0.9, loc='upper right',
                     bbox_to_anchor=(0.97, 0.95))  # Move legend higher up
        
        # Style adjustments (from original visualize_splits_3d)
        ax.view_init(elev=20, azim=45)
        ax.grid(False)
        ax.set_facecolor('white')
        fig.patch.set_facecolor('white')
        
        # Remove tight_layout and use figure adjustments instead (from original visualize_splits_3d)
        plt.subplots_adjust(left=0.05, right=0.95, bottom=0.05, top=0.95)
...
def plot_umap_embeddings_w_fc(embeddings, network_labels, conn_matrix=None, pc_values=None, std_edge_threshold=None, edge_threshold=None,
                        ...
                        fontsize=30, axis_fontsize=26, scatter_alpha=0.8, scatter_size=20, figsize=(10,7),
                        ...
                        schaefer_colors=False, enhance_legend=False):
    """
    ...
    Args:
        ...
        schaefer_colors: If True, uses Schaefer network colors for points (default False)
        enhance_legend: If True, uses full network names in legend (default False)
    """
    random_seed = 2
    np.random.seed(random_seed)

    # Define network color and name mappings
    network_to_color = {
        'Vis': '#7B3B7B',
        'SomMot': '#639CD6',
        'DorsAttn': '#D67AD6',
    ...
    if pc_values is not None:
        if network_gains is not None:
            ...
            size_scale = scatter_size * 7 # Increase size range
            ...
            plt.colorbar(scatter, label='Gene Exp PC1 Value', shrink=0.5)
            ...
            plt.colorbar(scatter, label='Gene Exp. PC1 Value')
        ...
        if network_gains is not None:
            ...
            # Use Schaefer colors or rainbow colors
            if schaefer_colors:
                for network in unique_networks:
                    mask = network_labels == network
                    color = network_to_color.get(network, '#000000')  # Default to black if network not found
                    plt.scatter(umap_embeddings[mask, 0], umap_embeddings[mask, 1],
                              color=color, alpha=scatter_alpha, s=scatter_size)
                    legend_label = network_fullnames[network] if enhance_legend else network
                    legend_elements.append(plt.Line2D([0], [0], marker='o', color='w',
                                                   markerfacecolor=color, label=legend_label, 
                                                   markersize=15))
            else:
                colors = plt.cm.rainbow(np.linspace(0.0, 1.0, len(unique_networks)))
...
def plot_coords_w_fc(coords, network_labels, dims=(0,1), conn_matrix=None, pc_values=None, std_edge_threshold=None, edge_threshold=None,
                    title=None, fontsize=30, axis_fontsize=26, scatter_alpha=0.8, scatter_size=20,
